...
class MainWindow(QMainWindow, form_class):  # 슬롯 클래스
            ...
            if self.alarm_sell_input.text() == '' or self.alarm_buy_input.text() == '':
                ...
                if self.alarmFlag == 0:
                    ...
                    if trade_price >= alarm1price:
                        self.alarmFlag = 1
                        print("매도가격도달!!!")
                        self.telegram_message(f"{self.ticker} 코인의 현재가격 : {trade_price:,}원")
                        self.telegram_message(f"지정해 놓은 매도가격인 {alarm1price:,}원에 도달하였습니다.")

                    if trade_price <= alarm2price:
                        self.alarmFlag = 1
                        print("매수가격도달!!!")
                        self.telegram_message(f"{self.ticker} 코인의 현재가격 : {trade_price:,}원")
                        self.telegram_message(f"지정해 놓은 매수가격인 {alarm2price:,}원에 도달하였습니다.")
    ...
    def telegram_message(self, message_text): # 텔레그램 메시지를 전송해주는 함수
        bot = telegram.Bot(token="본인의 토큰")
        chat_id = "본인의 chat id"

        asyncio.run(bot.sendMessage(chat_id=chat_id, text=message_text))
